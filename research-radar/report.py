...
def strongest(e):
    ...
    if choices:
        ...
        return m, d, v
    return max((m for m in e['markets'] if m.get('quality')), key=lambda m:m['volume']), None, None
...
def card(e, now, research=False, i=0):
    if not any(m.get('quality') for m in e['markets']):
        return ''
    m, d, v = strongest(e)
    ...
    body = f'<article class="card"><h3>{esc(e["title"])}</h3><p class="fact">{esc(facts(e))}</p>'
    body += '<div class="trend-preview">'+spark(m,now,COLORS[i%len(COLORS)])+'</div>'
    body += '<details class="event-details"><summary>展开概率明细与研究信息</summary>'
    body += f'<div class="eyebrow">{esc(e["topic"])} <span>{esc(e["platform"])}</span></div>'
    body += table(e)+'<p class="why">'+esc(reasons)+'</p>'
    if research:
        body += '<div class="research"><b>值得研究的问题</b><p>'+esc(e['question'])+'</p><small>规则生成的研究方向，不是新闻归因或投资结论。</small></div>'
    body += f'<footer>24h成交 {e["volume"]:,.0f} {unit} · {esc(deadline)}<br>采集 {clock(e["captured_at"])} 北京时间 · <a href="{esc(e["url"])}" target="_blank" rel="noopener noreferrer">原始市场／官方数据 ↗</a></footer></details></article>'
    return body
...
.collection-note{margin-top:36px;padding-top:18px;border-top:1px solid #d7e2d9;color:var(--muted);font-size:12px}.collection-note h2{font-size:15px;margin:0 0 10px}.collection-note p{margin:5px 0}.collection-note .status{font-size:12px}.event-details{border-top:1px solid #e6ece7;padding-top:12px;margin-bottom:0}.event-details summary{padding:4px 0;list-style:none;font-weight:600}.event-details summary::before{content:'＋';margin-right:8px}.event-details[open] summary::before{content:'−'}.event-details .eyebrow{margin:18px 0 8px}.trend-preview{margin-top:4px}.trend-preview .chart{display:block;max-height:210px}.card h3{margin-top:0}#signals h2{margin-top:18px}.method{font-size:12px}.method h2{font-size:17px;margin-top:0}
'''

def build_report(run, config, root:Path, site:Path, state):
    now = run['captured_at']
    # Presentation filtering only: retain every raw observation for future tracking.
    events = [{**e, 'markets': [m for m in e['markets'] if m.get('quality')]}
              for e in run['events'] if any(m.get('quality') for m in e['markets'])]
    day=date(now)
    ...
    current={e['key'] for e in run['events']}
    ...
    output={'schema_version':1,'date':day,'captured_at':now,'sources':run['sources'],'selected_keys':[e['key'] for e in selected],
            'watch_keys':[e['key'] for e in watches],'summaries':summaries,'events':run['events'],'model':{'enabled':False}}
    ...
    body=['<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">',
          ...
          f'<p class="meta">{day} 日报</p></header>',
          '<nav><a href="#signals">研究精选</a><a href="#watch">重点事件</a><a href="#archive">历史日报</a></nav>']
    coverage = ['<section id="coverage" class="collection-note"><h2>数据记录</h2>',
                f'<p>{clock(now)} 北京时间采集 · 每日09:00计划运行 · 零模型调用</p>',
                f'<p>{len(events)} 个可展示事件 · {len(watches)} 个重点事件 · {len(selected)} 个研究精选</p>']
    if failures:
        coverage.append('<p><b>本次数据不完整</b> · '+esc('、'.join(failures))+' 未完整取得。缺失不代表概率为零；本页不会把旧数据标成今日行情。</p>')
    coverage.append('<div class="status">'+''.join(f'<p><b>{esc(p)}</b> · {esc(s["status"])} · 扫描 {s["scanned_events"]:,} 个事件 · '+esc(s.get('scope','配置的监测范围'))+(' · '+esc(s['error']) if s.get('error') else '')+'</p>' for p,s in run['sources'].items())+'</div>')
    coverage.append('<p>变化单位为百分点；“—”表示缺少可靠历史。<a href="report.json">下载结构化数据</a></p></section>')
    body.extend(['<section id="signals"><h2>今日研究精选</h2>'])
    if selected:
    ...
    body.append('</section><section id="watch"><h2>重点事件持续跟踪</h2>')
...
<li>趋势图展示该事件中变化显著或成交活跃的一个合格结果；展开后仅显示达到筛选门槛的结果。市场到期时间不能直接当作政策公告时间。</li>
<li>低成交、宽价差或报价缺失的结果不在日报展示；后台仍保存观测和关注状态。所有有效结果均接近0%或100%的事件不进入新选题。新选题7日内重复需较上次推荐发生至少3个百分点的新变化。</li>
